...
if [[ -z "${NUGET_PUBLISH_INCLUDED}" ]]; then
    ...
    function check_should_publish_package() {
        ...
        log_debug "check_should_publish_package '${github_event_name}' '${github_event_action}' '${github_pr_merged}' '${github_ref}' '${package} '${needs_publish}'"
      ...
      if [[ "${needs_publish}" == "true" ]]; then
          if [[ ("${github_event_name}" == "push") || ("${github_event_name}" == "workflow_dispatch") ]]; then
              log_info "Changes that would affect the built component found in a '${github_event_name}' event => Should publish ${package}" true
              return 0
          elif [[ ("${github_event_name}" == "pull_request") && ( "${github_event_action}" == "closed" ) && ( "${github_pr_merged}" == "true") ]] ; then
              log_info "Changes that would affect the built component found in a merged pull request => Should publish ${package}" true
              return 0
          else
              log_info "Changes that would affect the built component found for a '${github_event_name}' event for '${github_ref}' (not a branch we expect to publish from) => will not publish ${package}" true
              return 1
          fi
      else
          log_info "No changes that would affect the built component found => will not publish ${package}" true
          return 1
      fi
    }
  ...
  function check_if_component_could_be_published() {
      ...
      local github_ref="$3"
      local artifactory_uri="$4"
      local artifactory_key="$5"

      log_debug "check_if_component_could_be_published '${package}' '${package_type}' '${github_ref}' '${artifactory_uri}' '<key>'"

      if ! (validate_mandatory_parameter "package" "${package}" && \
            validate_mandatory_parameter "package_type" "${package_type}" && \
      ...
      target_repo="$(get_component_target_nuget_repository "${branch_name}" "${package_type}")"
      other_repo="$(get_component_non_target_nuget_repository "${branch_name}" "${package_type}")"
      ...
      read -ra existing_repos <<< "$(get_existing_nuget_repos_for_package "${artifactory_uri}" "${artifactory_key}" "${package}" "${package_type}" )" || true
