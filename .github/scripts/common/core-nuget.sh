...
if [[ -z "${NUGET_INCLUDED}" ]]; then
  ...
  function get_component_target_nuget_repository() {
    ...
    if [[ "${branch}" == "master" || "${branch}" == "main" ]]; then
      ...
      log_error "unrecognised/unsupported branch: ${github_ref} - only master/main and feature/XXX branches are supported"
      exit 1
    fi
  }
  ...
  function get_component_non_target_nuget_repository() {
    ...
    local target_repo
    target_repo=$(get_component_target_nuget_repository "${github_ref}")
    local ret_val=$?
    if [[ ${ret_val} -ne 0 ]]; then
      exit "${ret_val}"
    fi

    if [[ "${target_repo}" == "${NUGET_COMPONENTS}" ]]; then
      echo "${NUGET_FEATURES}"
    else
      echo "${NUGET_COMPONENTS}"
    fi
